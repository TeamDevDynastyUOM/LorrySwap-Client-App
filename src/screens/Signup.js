...
const RoleSelectionCheckBox = ({ values, setFieldValue }) => {
  const toggleRole = (role) => {
    ...
    if (role === 'EconomicCenterSeller') {
      if (updatedRoles.includes(role)) {
        updatedRoles = updatedRoles.filter((r) => r !== role);
      } else {
        updatedRoles = updatedRoles.filter((r) => r !== 'MarketSeller');
        updatedRoles.push(role);
      }
    } else if (role === 'MarketSeller') {
      if (updatedRoles.includes(role)) {
        updatedRoles = updatedRoles.filter((r) => r !== role);
      } else {
        updatedRoles = updatedRoles.filter((r) => r !== 'EconomicCenterSeller');
        updatedRoles.push(role);
      }
    } else {
      if (updatedRoles.includes(role)) {
  ...
  return (
    <View style={{...styles.genderStyle, alignItems: 'flex-start',}}>
      <Text style={{...styles.txt, marginTop: 15}}>Roles</Text>
      <View style={{...styles.genderSection, flexDirection: 'column'}}>
        <View style={{...styles.checkboxContainer, marginLeft: -10, marginBottom: -15  }}>
          <Checkbox.Item
            status={values.role.includes('EconomicCenterSeller') ? 'checked' : 'unchecked'}
            onPress={() => toggleRole('EconomicCenterSeller')}
            color="#004344"
          />
          <Text style={{ ...styles.txt, marginLeft: -15 }}>Economic Center Seller</Text>
        </View>

        <View style={{...styles.checkboxContainer, marginLeft: -10, marginBottom: -15 }}>
          <Checkbox.Item
            status={values.role.includes('MarketSeller') ? 'checked' : 'unchecked'}
            onPress={() => toggleRole('MarketSeller')}
            color="#004344"
          />
          <Text style={{ ...styles.txt, marginLeft: -15 }}>Market Seller</Text>
        </View>

        <View style={{...styles.checkboxContainer, marginLeft: -10 }}>
          <Checkbox.Item
            status={values.role.includes('CargoFinder') ? 'checked' : 'unchecked'}
            onPress={() => toggleRole('CargoFinder')}
            color="#004344"
          />
          <Text style={{ ...styles.txt, marginLeft: -15 }}>Cargo Finder</Text>
        </View>
      </View>
    </View>
  );
